 ...
 * plan Table class
 */
class JmsTableplan extends JTable
{
	/**
	 * Constructor
	 *
	...
	{
		parent::__construct('#__jms_plans', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (isset($array['articles']) && is_array($array['articles'])) {
			$registry = new JRegistry();
			$registry->loadArray($array['articles']);
			$array['articles'] = (string)$registry;
		}
		...
		if (isset($array['components']) && is_array($array['components'])) {
			$registry = new JRegistry();
			$registry->loadArray($array['components']);
			$array['components'] = (string)$registry;
		}
		
		if (!isset($array['articles'])) {
            $array['articles'] = 0; 
			$array['article_type'] = 0;
		} else {
		...
		if (!isset($array['categories'])) {
            $array['categories'] = 0; 
			$array['category_type'] = 0;
		} else {
			$array['category_type'] = 1;
		}
		
		if (!isset($array['components'])) {
            $array['components'] = 0; 
			$array['component_type'] = 0;
		} else {
			$array['component_type'] = 1;
		}
		...
		if ($array['period_type'] == 5) {
			$array['period'] = 0;
			$array['number_of_installments'] = 0;
		}
