 ...
 * @version 	$Id: productimport.php 2029 2012-06-14 14:11:17Z RolandD $
 */
...
class CsviModelProductimport extends CsviModelImportfile {
			...
			foreach ($this->csvi_data as $name => $value) {
				...
				switch ($name) {
					...
					case 'shopper_group_name':
						$this->shopper_group_id = $this->helper->getShopperGroupId($value);
						break;
	...
	private function _loadCustomFields() {
		...
		$query->select("virtuemart_custom_id AS id, LOWER(TRIM(custom_title)) AS title");
		$query->from('#__virtuemart_customs');
		$query->where('field_type = '.$db->quote('S'));
		$db->setQuery($query);
	...
	private function _processRelatedProducts() {
		...
		foreach ($relatedproducts AS $key => $relatedproduct) {
			$entries[] = "(".$db->Quote($this->product_sku).", ".$db->Quote($relatedproduct).")";
		}
	...
	private function _postProcessRelatedProducts() {
		...
		if (!empty($relations)) {
			...
			$db->setQuery("TRUNCATE ".$db->quoteName('#__csvi_related_products'));
			$db->query();
		}
		else {
		...
		if ($template->get('process_image', 'image', false)) {
			if (!is_null($this->file_url) || $template->get('auto_generate_image_name', 'image', false)) {
				...
				foreach ($images as $key => $image) {
					...
					if ($template->get('auto_generate_image_name', 'image')) $this->_createImageName($img_counter);
	...
	private function _processCustomAvailableFields() {
		...
		foreach ($this->_customfields as $field) {
			...
			if (isset($this->$title)) {
				...
				$query2->delete('#__virtuemart_product_customfields');
				$query2->where('virtuemart_product_id = '.$this->virtuemart_product_id);
				$query2->where('virtuemart_custom_id = '.$field->id);
				$db->setQuery($query2);
				$db->query();
				
				// Insert query
				$query = $db->getQuery(true);
				$query->insert('#__virtuemart_product_customfields');
				$query->columns(array('virtuemart_product_id', 'virtuemart_custom_id', 'custom_value'));
				$query->values($this->virtuemart_product_id.', '.$field->id.','.$db->quote($this->$title));
				$db->setQuery($query);
				$db->query();
