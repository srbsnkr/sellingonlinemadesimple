 ...
 * @version 	$Id: maintenance.php 2018 2012-06-04 10:52:33Z RolandD $
 */
	...
	public function getBackupTemplates() {
		...
		if ($fp) {
			...
			foreach ($templates as $template) {
				...
				$query->select($db->quoteName('combine'));
						...
						if ($fp) {
							while (($data = fgetcsv($fp, 0, ",")) !== FALSE) {
								...
								if ($db->query()) {
									...
									foreach ($fields as $field) {
										$db->setQuery("INSERT IGNORE INTO #__csvi_template_fields (".$db->quoteName('template_id').", ".$db->quoteName('ordering').", ".$db->quoteName('field_name').", ".$db->quoteName('column_header').", ".$db->quoteName('default_value').", ".$db->quoteName('process').", ".$db->quoteName('combine').", ".$db->quoteName('sort').")
												VALUES (".$db->Quote($template_id).", ".$db->Quote($field->ordering).", ".$db->Quote($field->field_name).", ".$db->Quote($field->column_header).", ".$db->Quote($field->default_value).", ".$db->Quote($field->process).", ".$db->Quote($field->combine).", ".$db->Quote($field->sort).")");
										if ($db->query()) {
