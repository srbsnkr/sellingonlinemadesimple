 ...
 * @version 	$Id: templates.php 2013 2012-05-31 19:08:47Z RolandD $
 */
	...
	public function save($data) {
		...
		if ($table->store()) {
			...
			if ($jinput->get('old_template_id', 0)) {
				$db = JFactory::getDbo();
				$query = "INSERT INTO #__csvi_template_fields (template_id, ordering, field_name, column_header, default_value,`process`,`combine`, `sort`) 
					(SELECT '".$table->id."', ordering, field_name, column_header, default_value,`process`,`combine`, `sort` FROM #__csvi_template_fields WHERE template_id = ".$jinput->get('old_template_id').")";
				$db->setQuery($query);
				$db->query();
			}
		}
