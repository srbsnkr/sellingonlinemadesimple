 ...
 * @version		$Id: importfile.php 2025 2012-06-12 10:23:21Z RolandD $
 */
...
class CsviModelImportfile extends JModel {
	...
	*/
			...
			if ($template->get('add_extra_fields', 'general', false)) {
				...
				if (!empty($fields)) {
					foreach ($fields as $fid => $field) {
						...
						// Collect the data
						$csvifields[$name]['name'] = $name;
						$csvifields[$name]['order'] = ++$curfields;
						$csvifields[$name]['default_value'] = $field->default_value;
						$csvifields[$name]['published'] = $field->process;
						$csvifields[$name]['combine'] = $field->combine;
						$csvifields[$name]['replace'] = $field->replace;
		...
		// Use the fields assigned to the template
			...
			if (!empty($fields)) {
				foreach ($fields as $fid => $field) {
					...
					// Collect the data
					$csvifields[$name]['name'] = $name;
					$csvifields[$name]['order'] = ($field->ordering-1);
					$csvifields[$name]['default_value'] = $field->default_value;
					$csvifields[$name]['published'] = $field->process;
					$csvifields[$name]['combine'] = $field->combine;
					$csvifields[$name]['replace'] = $field->replace;
	...
	protected function convertDate($date, $type='sql') {
		...
		list($date_part, $time_part) = explode(' ', $new_date);
