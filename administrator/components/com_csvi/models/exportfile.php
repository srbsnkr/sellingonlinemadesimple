 ...
 * @version 	$Id: exportfile.php 1956 2012-04-01 20:33:26Z RolandD $
 */
...
class CsviModelExportfile extends JModel {
	...
	private function _exportDetails() {
		...
		$addheader = true;
		foreach ($export_fields as $column_id => $field) {
			if ($addheader) $csvilog->addDebug(JText::sprintf('COM_CSVI_DEBUG_EXPORT_FIELD', ($field->column_header) ? $field->column_header : $field->field_name));
			if ($field->combine) $addheader = false;
			else $addheader = true;
		}
	...
	private function _outputEnd() {
		...
		switch ($template->get('exportto', 'general')) {
			...
			case 'toftp':
				...
				$ftp->chdir($template->get('ftproot', 'general', '/'));
	...
	protected function addExportField($combine=false, $data, $fieldname, $column_header, $cdata=false) {
		...
		if (!$combine) {
			if (!empty($this->_outputfield)) {
				...
				$data = implode($template->get('combine_char', 'general', ''), $this->_outputfield);
