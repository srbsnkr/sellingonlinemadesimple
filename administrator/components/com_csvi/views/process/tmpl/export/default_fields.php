 ...
 * @version 	$Id: default_fields.php 1956 2012-04-01 20:33:26Z RolandD $
 */
	...
	<div id="export_fields">
		<table id="newfieldlist" class="adminlist">
			<thead>
				<tr>
					...
					<th class="title"><?php echo JText::_('COM_CSVI_COMBINE_FIELD'); ?>
					</th>
			...
			<tbody>
				<tr>
					...
					<!-- Combine field -->
					<td id="newfield_combinefield"><?php echo CsviHelper::getYesNo('_combine_field', '0', '', '_combine_field_default'); ?>
					</td>
					<!-- Sort field -->
		...
		<div id="toolbar" class="toolbar-list">
			<ul>
				...
				<li id="toolbar-edit" class="button"><?php echo JHtml::_('link', '#', '<span class="icon-32-csvi_edit_32">&nbsp;</span>'.JText::_('COM_CSVI_EDIT'), 'id="editlink" onclick="CsviTemplates.getHref(\''.JURI::root().'\');" class="modal" rel="{handler: \'iframe\', size: {x: 500, y: 450}}"'); ?>
				</li>
		...
		<table id="fieldslist" class="adminlist">
			<thead>
				<tr>
					...
					<th class="title" width="5%"><?php echo JText::_('COM_CSVI_COMBINE_FIELD') ?>
					</th>
				</tr>
			</thead>
			<tfoot>
			...
			<tbody>
				...
				if (is_array($export_fields)) {
					foreach ($export_fields as $key => $field) {
						?>
				<tr>
					<td align="center"><?php echo JHtml::_('grid.id', $key, $field->field_id); ?>
					</td>
					...
					<td><?php echo JHtml::_('link', JRoute::_('index.php?option=com_csvi&task=templatefield.edit&tmpl=component&id='.$field->field_id.'&template_id='.$this->template->getId()), $field->field_name, 'class="modal" rel="{handler: \'iframe\', size: {x: 500, y: 450}}"'); ?></td>
					...
					<td class="center" id="field<?php echo $field->field_id; ?>">
						<?php if (!$field->combine) { ?>
							<a title="<?php echo JText::_('JLIB_HTML_PUBLISH_ITEM'); ?>" onclick="return listItemTask('<?php echo $field->field_id; ?>','templatefield.combine')" href="javascript:void(0);" class="jgrid"><span class="state unpublish"><span class="text"><?php echo JText::_('JUNPUBLISHED'); ?></span></span></a>
						<?php }
							else if ($field->combine) { ?>
							<a title="<?php echo JText::_('JLIB_HTML_UNPUBLISH_ITEM'); ?>" onclick="return listItemTask('<?php echo $field->field_id; ?>','templatefield.uncombine')" href="javascript:void(0);" class="jgrid"><span class="state publish"><span class="text"><?php echo JText::_('JPUBLISHED'); ?></span></span></a>
						<?php } ?>
					</td>
				</tr>
				<?php }
