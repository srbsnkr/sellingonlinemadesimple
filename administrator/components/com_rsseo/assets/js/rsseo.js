function sitemapGenerate(SitemapFrequency, SitemapModification, SitemapPriority,SitemapNew,protocol)
{
	xml=buildXmlHttp();
	xml.onreadystatechange=function()
    {
		if(xml.readyState==4)
		{
			if(xml.responseText != 'finish')
			{
				...
				xml.open('GET','index.php?option=com_rsseo&task=sitemapGenerate&controller=sitemap&SitemapFrequency=' + SitemapFrequency + '&SitemapModification=' + SitemapModification + '&SitemapPriority=' + SitemapPriority + '&protocol=' + protocol + '&randomTime=' + Math.random(), true);
				xml.send(null);
			}else{
	...
	xml.open('GET','index.php?option=com_rsseo&task=sitemapGenerate&controller=sitemap&SitemapFrequency=' + SitemapFrequency + '&SitemapModification=' + SitemapModification + '&SitemapPriority=' + SitemapPriority + '&SitemapNew=' + SitemapNew + '&protocol=' + protocol + '&randomTime=' + Math.random(), true);
	xml.send(null);
...
function refreshKeyword(root,cid)
{
	...
	xml.open('GET','index.php?option=com_rsseo&task=refreshkeyword&controller=keywords&cid=' + cid + '&randomTime=' + Math.random(), true);
	xml.send(null);
...
function refreshCompetitor(root,cid)
{
	...
	xml.onreadystatechange=function()
    {
		if(xml.readyState==4)
		{
			...
			document.getElementById('dmoz' + cid).innerHTML=params[19];
			
		}
    }
	xml.open('GET','index.php?option=com_rsseo&task=refreshCompetitor&controller=competitors&cid=' + cid + '&randomTime=' + Math.random(), true);
	xml.send(null);
	...
	document.getElementById('dmoz' + cid).innerHTML='...';
...
function crawl(start,idPage)
{
	...
	xml.open('GET','index.php?option=com_rsseo&task=crawl&controller=crawler&start=' + start +'&idPage='+ idPage + '&randomTime=' + Math.random(),true);
	xml.send(null);
}


function rss_restore(cid)
{
	document.getElementById('rss_loader').style.display='';
	xml=buildXmlHttp();
	xml.onreadystatechange=function()
	{
		if(xml.readyState==4)
		{
			...
			{
				document.getElementById('rss_loader').style.display='none';
				location.reload();
			}
		}
	}
	xml.open('GET','index.php?option=com_rsseo&task=restore&controller=pages&cid=' + cid[cid.length - 1] + '&tmpl=component&randomTime=' + Math.random(), true);
	xml.send(null);
}


function rss_refresh(cid)
{
	document.getElementById('rss_loader').style.display='';
	xml=buildXmlHttp();
	xml.onreadystatechange=function()
	...
	xml.open('GET','index.php?option=com_rsseo&task=crawl&controller=crawler&start=0&idPage='+ cid[cid.length - 1] + '&randomTime=' + Math.random(),true);
	xml.send(null);
}



function rss_doCheck(value)
{
	document.getElementById('rss_hide1').style.display='none';
	document.getElementById('rss_hide2').style.display='none';
	document.getElementById('rss_hide3').style.display='none';
	document.getElementById('rss_img_loading').style.display='';
	xml=buildXmlHttp();
	xml.onreadystatechange=function()
	{
		if(xml.readyState==4)
		{
			...
			document.getElementById('rss_hide1').style.display='';
			document.getElementById('rss_hide2').style.display='';
			document.getElementById('rss_hide3').style.display='';
			document.getElementById('rss_img_loading').style.display='none';
			document.getElementById('rss_size').innerHTML = response[0];
			document.getElementById('rss_time').innerHTML = response[1];
			
		}
	}
	xml.open('GET','index.php?option=com_rsseo&task=checktimesize&controller=pages&cid=' + value + '&tmpl=component&randomTime=' + Math.random(), true);
	xml.send(null);
}

function rss_add_task(root,task,acc,start,end)
{
	$('rss_loader').style.display = '';
	
	var url = 'administrator/index.php?option=com_rsseo';
	var params = 'task='+task	
	if (typeof(acc) != 'undefined' && task == 'gaaccount')
		params += '&account='+acc;
	if (typeof(start) != 'undefined' && task == 'gaaccount')
		params += '&start='+start;
	if (typeof(end) != 'undefined' && task == 'gaaccount')
		params += '&end='+end;
	params += '&randomTime='+Math.random();
	xml = buildXmlHttp();
	
	xml.open('POST',root + url,false);
	xml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xml.setRequestHeader("Content-length", params.length);
	xml.setRequestHeader("Connection", "close");
	xml.send(params);
	
	$(task).innerHTML = xml.responseText;
	$('rss_loader').style.display = 'none';
