 ...
 */
class joomailermailchimpintegrationsControllerCampaigns extends joomailermailchimpintegrationsController
{
	/**
	 * display the opened list
	 * @return void
	 */
	function opened()
	{
	 ...
	 * @param Array $headers
	 */
	function httpRequest($url, $get_variables=null, $post_variables=null, $headers=null)
	{
	...
	function analytics(){
		...
		$url = 'index.php?option=com_joomailermailchimpintegration&view=analytics360';
	...
	function sendShareReport(){
		...
		$header_type = JRequest::getVar( 'headerType', '', 'post', 'string' );
		$title = JRequest::getVar( 'title', '', 'post', 'string' );
		$logoUrl = JRequest::getVar( 'logoUrl', false, 'post', 'string', JREQUEST_ALLOWRAW );
		if($logoUrl){
			$header_data = $logoUrl;
		} else {
			$header_data = $title;
		...
		$secure = JRequest::getVar( 'secure', 0, 'post', 'string' );
		$password = JRequest::getVar( 'password', '', 'post', 'string', JREQUEST_ALLOWRAW );
		
		// colors
		$colors = array();
		$colors['bg_color'] = str_replace('#','', JRequest::getVar( 'bg_color', '', 'post', 'string' ) );
		$colors['header_color'] = str_replace('#','', JRequest::getVar( 'header_color', '', 'post', 'string' ) );
		$colors['current_tab'] = str_replace('#','', JRequest::getVar( 'current_tab', '', 'post', 'string' ) );
		$colors['current_tab_text'] = str_replace('#','', JRequest::getVar( 'current_tab_text', '', 'post', 'string' ) );
		$colors['normal_tab'] = str_replace('#','', JRequest::getVar( 'normal_tab', '', 'post', 'string' ) );
		$colors['normal_tab_text'] = str_replace('#','', JRequest::getVar( 'normal_tab_text', '', 'post', 'string' ) );
		$colors['hover_tab'] = str_replace('#','', JRequest::getVar( 'hover_tab', '', 'post', 'string' ) );
		$colors['hover_tab_text'] = str_replace('#','', JRequest::getVar( 'hover_tab_text', '', 'post', 'string' ) );
		
		$opts = array();
		$opts['header_type'] = $header_type;
		$opts['header_data'] = $header_data;
		$opts['secure'] = $secure;
		$opts['password'] = $password;
		$opts['to_email'] = $email;
		$opts['theme'] = $colors;
		
		$result = $mc->campaignShareReport($cid, $opts);
	...
	function clearReportsCache(){
	    ...
	    $group = 'joomailermailchimpintegrationReports';
	    $path = JPATH_ADMINISTRATOR.DS.'cache'.DS.$group;
	    if( !JFolder::exists( $path ) || JFolder::delete( $path )){
		$response['success'] = true;	
	    } else {
		$response['success'] = false;
		$response['html'] = 'Error: deleting cache failed';
	    }
