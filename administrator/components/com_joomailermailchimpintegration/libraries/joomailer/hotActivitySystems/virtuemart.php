...
require_once(JPATH_COMPONENT_ADMINISTRATOR . DS . 'libraries' . DS . 'joomailer' . DS . 'interfaces' . DS . 'hotActivity.php');
...
class joomailerVirtuemartActivity  implements hotActivity {
	...
	public function getActivity() {
    	...
    	$query = 'SELECT FROM_UNIXTIME(o.cdate) as crdate, i.order_item_name as title, FORMAT(i.product_item_price,2) as price, "virtuemart" AS joomailerProductCategory'
				. ' FROM ' . $db->nameQuote('#__vm_orders') . ' AS o '
				. ' LEFT JOIN ' . $db->nameQuote('#__vm_order_item') . ' AS i '
				. ' ON o.order_id = i.order_id'
				. ' WHERE o.user_id = '. $db->Quote($userId)
				. ' AND i.order_status = ' . $db->Quote('S')
	...
	public function getAllProducts() {
		$db = JFactory::getDBO();
		$query = 'SELECT count(product_id) as count FROM ' . $db->nameQuote('#__vm_product')
				. ' WHERE ' . $db->nameQuote('product_publish') . '=' . $db->Quote('Y')
				. ' AND ' . $db->nameQuote('product_parent_id') . '=' . $db->Quote(0);
	...
	public function getProductsUserBought() {
		...
		$now = $date->toMySQL();
		$query = 'SELECT count(i.order_item_id) as count FROM ' .  $db->nameQuote('#__vm_order_item') . ' AS i'
				. ' LEFT JOIN ' . $db->nameQuote('#__vm_orders') . ' AS o '
				. ' ON o.order_id = i.order_id'
				. ' WHERE o.user_id = '. $db->Quote($userId)
				. ' AND i.order_status = ' . $db->Quote('S');
	...
	private function countProductsPerUser() {
		...
		$query = 'SELECT COUNT(i.order_item_id) AS count, u.email AS email '
				. ' FROM ' . $db->nameQuote('#__vm_order_item') . ' AS i'
				. ' LEFT JOIN ' . $db->nameQuote('#__vm_orders') . ' AS o'
				. ' ON i.order_id = o.order_id'
				. ' LEFT JOIN ' . $db->nameQuote('#__users') . ' AS u'
				. ' ON o.user_id = u.id'
				. ' WHERE i.order_status = ' . $db->Quote('S')
				. ' GROUP BY ' . $db->nameQuote('u.email');
		$db->setQuery($query);
