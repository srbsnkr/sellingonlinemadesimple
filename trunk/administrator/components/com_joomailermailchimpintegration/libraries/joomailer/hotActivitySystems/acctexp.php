...
class joomailerAcctexpActivity  implements hotActivity {
	...
	public function getActivity() {
		...
		$query = 'SELECT s.signup_date as crdate, p.name as title, p.params as params, "AEC" AS joomailerProductCategory'
				. ' FROM ' . $db->nameQuote('#__acctexp_subscr') . ' AS s '
				. ' LEFT JOIN ' . $db->nameQuote('#__acctexp_plans') . ' AS p '
				. ' ON s.plan = p.id'
				. ' WHERE s.userid = '. $db->Quote($userId)
				. ' AND status = ' . $db->Quote('Active')
	...
	private function getAllSubscriptions() {
		$db = JFactory::getDBO();
		$query = 'SELECT count(id) as count FROM ' . $db->nameQuote('#__acctexp_plans')
				. ' WHERE ' . $db->nameQuote('active') . '=' . $db->Quote(1);
	...
	private function getSubscriptionsUserHas() {
		...
		$now = $date->toMySQL();
		$query = 'SELECT count(id) as count FROM ' . $db->nameQuote('#__acctexp_subscr')
