...
<div class="note">
    <?php echo JText::sprintf( 'JM_REPORTS_DATE', strftime("%Y-%m-%d %H:%M", $this->cacheDate) ).' '; ?> <a href="javascript:clearReportsCache()"><?php echo JText::_( 'JM_CLICK_HERE_TO_REFRESH' ); ?></a>
    <span id="cacheLoader" style="display:inline-block;height:16px;position:relative;top:2px;visibility:hidden;width:16px;"><img src="<?php echo JURI::root();?>administrator/components/com_joomailermailchimpintegration/assets/images/loader_16.gif" style="margin: 0 0 0 10px;"/></span>
</div>
...
function clearReportsCache(){
    ...
    var url = baseUrl + "index.php?option=com_joomailermailchimpintegration&action=AJAX&controller=campaigns&format=raw&task=clearReportsCache";
    var data = new Object();
    jQuery("#cacheLoader").css( 'visibility', 'visible' );
    doAjaxTask(url, data, function(postback){	if(postback.success){
						    joomailermailchimpintegration_ajax_loader();
						    location.reload(true);
						} else {
						    jQuery("#cacheLoader").css( 'width', '').css('position', '');
						    jQuery("#cacheLoader").html( postback.html );
						}
					    });
...
<form action="index.php?option=com_joomailermailchimpintegration&view=campaigns" method="post" name="adminForm">
<table width="100%">
    <tbody>
	<tr>
...
<table class="adminlist">
	...
	<tr>
	    ...
	    <th nowrap="nowrap"><?php echo JText::_( 'JM_CAMPAIGN_NAME' ).' ('.JText::_( 'JM_SUBJECT' ).')'; ?>
	    </th>
	    <th width="125" nowrap="nowrap"><?php echo JText::_( 'JM_SENT_DATE' ); ?>
	    </th>
	    <th width="100" nowrap="nowrap"><?php echo JText::_( 'JM_TOTAL_RECIPIENTS' ); ?>
	    </th>
	    <th width="100" nowrap="nowrap"><?php echo JText::_( 'JM_UNIQUE_OPENS' ); ?>
	    </th>
	    <th width="80" nowrap="nowrap"><?php echo JText::_( 'JM_BOUNCED' ); ?></th>
	    <th width="80" nowrap="nowrap"><?php echo JText::_( 'JM_UNIQUE_CLICKS' ); ?>
	    </th>
	    <th width="80" nowrap="nowrap"><?php echo JText::_( 'JM_UNSUBSCRIBES' ); ?>
	    </th>
	    <th width="80" nowrap="nowrap"><?php echo JText::_( 'JM_SHARE' ); ?>
	    </th>
	</tr>
    </thead>
	...
	foreach ($this->items as $id => $row)
	{
		...
		//$summary =& $model->getSummary( $row['CampaignID'] );
		...
		*/
		...
		. '</span> '
		. '<br /><a href="index.php?option=com_joomailermailchimpintegration&view=campaigns&layout=abuse&cid='.$id.'">'
		...
		if(is_array($countries)){
		    ...
		    foreach($countries as $c){
			    ...
			    $graph[$i] .= '<img src="'.JURI::root().'administrator/components/com_joomailermailchimpintegration/assets/images/flags/'.strtolower($c['code']).'.png" /></td><td>';
		...
		$doc->addScriptDeclaration("function showGraph_".$i."(){
	...
	<tr class="<?php echo "row$k"; ?>" id="<?php echo "row$i";?>" style="cursor: pointer;" onclick="showGraph_<?php echo $i;?>();">
		...
		<td align="center">
		    <input type="checkbox" name="cid[]" class="cid" id="<?php echo "cb$k";?>" value="<?php echo $id;?>" onclick="isChecked(this.checked);" />
		</td>
		<td>
		...
		<td align="center">
			...
			<img src="../administrator/components/com_joomailermailchimpintegration/assets/images/share.png" alt="Share" title="Share" height="17"/>
		    </a>
		</td>
	...
	<?php $doc->addScriptDeclaration($chartData); ?>

	<?php $active = JRequest::getVar('active', 0, 'get', 'int'); ?>
	<script type="text/javascript">showGraph_<?php echo $active;?>();</script>
...
<div style="display: none;"><?php echo $graphPreload;?></div>
