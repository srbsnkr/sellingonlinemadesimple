 ...
 * @version 	$Id: default.php 1981 2012-04-29 12:11:30Z RolandD $
 */
...
//Add new field to fields list
 jQuery("#addRow").click(function() {
 	...
 	var selectedfields = [];
 	jQuery('#_replace_field_default option').each(function(index, option) {
 	    if (jQuery(option).attr('selected')) {
 	    	selectedfields[index] = jQuery(option).val();
 	    }
 	});
 	...
 	if (template_id > 0) {
 		jQuery.ajax({
 			...
 			data: 'option=com_csvi&task=templatefield.storetemplatefield&format=json&template_id='+template_id
 					...
 					+'&combine='+jQuery('#_combine_field_default').val()
 					+'&sort='+jQuery('#_sort_field_default').val()
 					+'&replacefields='+selectedfields.join(','),
 			success: function(data) {
 				window.location = "index.php?option=com_csvi&view=process&template_id="+template_id;
 			},
 			error:function (request, status, error) {
