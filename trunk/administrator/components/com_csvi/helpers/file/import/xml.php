 ...
 * @version 	$Id: xml.php 1994 2012-05-22 06:18:05Z RolandD $
 */
...
class XmlFile extends CsviFile {
	...
	public function processFile() {
		...
		if ($this->fp == false) {
			...
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_CSVI_XML_INVALID_TABLE'), 'error');
			return false;
		}
		return true;
	}
					...
					if ($this->data->nodeType == XMLReader::ELEMENT) {
						...
						if (empty($this->_xml_schema)) {
							...
							if (empty($this->_xml_schema[$record_name]['nodes'][$current_path]['field'])) {
								...
								if (isset($this->_xml_schema[$record_name]['nodes'][$current_path]['csvi'])) {
									$action = $this->_xml_schema[$record_name]['nodes'][$current_path]['csvi'];
								}
								else $action = '';
								...
								$xmldata = $this->data->readInnerXML();
								$xmldata = str_ireplace(array('<![CDATA[',']]>'), '', $xmldata);
								$fieldname = $this->_xml_schema[$record_name]['nodes'][$current_path]['field'];
								switch ($action) {
									case 'combine':
										// Check if the field already exists
										$key = array_search($fieldname, $this->_xml_fields);
										if ($key === false) {
											$this->_xml_fields[] = $fieldname;
											$this->_xml_data[] = $xmldata;
										}
										else {
											$this->_xml_data[$key] .= "\n".$xmldata;
										...
										break;
									case 'add':
										// Set the index if not yet done so
										if (!isset($this->_xml_schema[$record_name]['nodes'][$current_path]['index'])) $this->_xml_schema[$record_name]['nodes'][$current_path]['index'] = 0;
										else $this->_xml_schema[$record_name]['nodes'][$current_path]['index']++;
										
										// Get the index
										$index = $this->_xml_schema[$record_name]['nodes'][$current_path]['index'];
										
										
										// Get the fieldname
										$fieldname = $this->_xml_schema[$record_name]['nodes'][$current_path]['field'][$index];
			...
			if ($xml_schema->nodeType == XMLReader::ELEMENT) {
				...
				if (empty($current_record)) {
					...
					while ($xml_schema->moveToNextAttribute()) {
						...
						$value = trim($xml_schema->value);
						if (!empty($value)) {
							if( $this->_isXmlFieldNameValid($xml_schema->value) ) {
								$this->_xml_schema[$current_record]['attrs'][strtolower($xml_schema->name)] = $value;
							}
							else {
								$app = JFactory::getApplication();
								$app->enqueueMessage(JText::sprintf('COM_CSVI_XML_FILE_MAP_NO_REFERENCE', $xml_schema->name), 'error');
								$csvilog->AddStats('incorrect', JText::sprintf('COM_CSVI_XML_FILE_MAP_NO_REFERENCE', $xml_schema->name));
								$xml_schema->close();
								return false;
							}
						}
								...
								switch ($name) {
									case 'csvi':
										switch ($value) {
											case 'combine':
											case 'add':
												$this->_xml_schema[$current_record]['nodes'][$current_path]['csvi'] = $value;
												break;
										}
										break;
									default:
										if ($this->_isXmlFieldNameValid($value)) {
											$this->_xml_schema[$current_record]['nodes'][$current_path]['attrs'][strtolower($name)] = $value;
										}
										else {
											$app = JFactory::getApplication();
											$app->enqueueMessage(JText::sprintf('COM_CSVI_XML_FILE_MAP_NO_REFERENCE', $xml_schema->name), 'error');
					...
					// Get the subnodes
					$sub_node_content = trim($xml_schema->readInnerXML());
					...
					if (strstr($sub_node_content, '<') === false) {
						 ...
						 */
						$action = isset($this->_xml_schema[$current_record]['nodes'][$current_path]['csvi']) ? $this->_xml_schema[$current_record]['nodes'][$current_path]['csvi'] : '';
						if ($this->_isXmlFieldNameValid($sub_node_content)) {
							if ($action == 'add') {
								$this->_xml_schema[$current_record]['nodes'][$current_path]['field'][] = $sub_node_content;
							}
							else $this->_xml_schema[$current_record]['nodes'][$current_path]['field'] = $sub_node_content;
						}
						else {
	...
	*/
	 ...
	 * @since 		1.0
	 */
	private function _getNodesMap() {
		$jinput = JFactory::getApplication()->input;
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
	
		// Get all categories but not the ROOT
		$query->select('a.title AS text, a.level, a.csvifield');
		...
		$query->where('a.lft >= '.$jinput->get('lft', 0, 'int'));
		$query->where('a.rgt <= '.$jinput->get('rgt', 107, 'int'));
		$query->group('a.id');
		...
		for ($i = 0, $n = count($options); $i < $n; $i++) {
			...
			if ($options[$i]->level > $curlevel) {
				$curlevel++;
			}
			else if ($options[$i]->level < $curlevel) {
			...
			if ($options[$i]->level == 1) {
				$xmllayout .= '<'.$options[$i]->text.'>';
			}
			else {
				$xmllayout .= '<'.$options[$i]->text.'>'.$options[$i]->csvifield;
			}
	
			// Add the level
			$levels[] = $options[$i]->text;
		}
