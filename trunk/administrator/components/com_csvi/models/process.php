 ...
 * @version		$Id: process.php 2030 2012-06-14 15:06:23Z RolandD $
 */
...
class CsviModelProcess extends JModelForm {
	...
	public function testFtp() {
		$jinput = JFactory::getApplication()->input;
		$ftphost = $jinput->get('ftphost');
		$ftpport = $jinput->get('ftpport');
		$ftpusername = $jinput->get('ftpusername');
		$ftppass = $jinput->get('ftppass');
		$ftproot = $jinput->get('ftproot');
		$ftpfile = $jinput->get('ftpfile');
			...
			if ($ftp->chdir($ftproot)) {
				if ($action == 'import') {
					...
					if (!in_array($ftpfile, $files)) {
						$this->setError(JText::_('COM_CSVI_FTP_FILE_NOT_FOUND'));
						return false;
					}
					else return true;
				}
				...
				$this->setError(JText::_('COM_CSVI_FTP_FOLDER_NOT_FOUND'));
				return false;
			}
		}
		else {
